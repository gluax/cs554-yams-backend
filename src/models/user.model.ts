...
export interface IUserModel extends Model<IUser> {
   newUser(
      newUser: IUser,
      callback: (err: Error, user: IUser) => void
   ): Promise<void>;
   getUserByUsername(
      username: string,
      callback: (err: Error, user: IUser) => any
   ): void;
   getUserById(id: string, callback: (err: Error, user: IUser) => any): void;
   comparePassword(
      possiblePass: string,
      hash: string,
      callback: (err: Error, match: boolean) => any
   ): void;
   updateUser(
      id: any,
      updates: any,
...
UserSchema.static(
   'getUserByUsername',
   (username: string, callback: (err: Error, user: IUser) => any): void => {
...
UserSchema.static(
   'getUserById',
   (id: string, callback: (err: Error, user: IUser) => any): void => {
